...
const tweetsIniciales = [
  {
    id: 1,
    author: "Ada Lovelace",
    text: "¡El motor analítico teje patrones algebraicos igual que el telar de Jacquard! 💻✨",
    likes: 42,
    date: "Hace 2h",
...
function App() {
  ...
  return (
    <div className="twitter-layout">
      ...
      <Routes>
        <Route path="/" element={<Home user={user} tweets={tweets} onAddTweet={handleAddTweet} onLike={handleLike} onDelete={handleDeleteTweet} />} />
        <Route path="/login" element={user ? <Navigate to="/" replace /> : <Login onLogin={handleLogin} />} />
        <Route path="/profile" element={user ? <Profile user={user} tweets={tweets} onLike={handleLike} onDelete={handleDeleteTweet} /> : <Navigate to="/login" replace />} />
      </Routes>
    </div>
  );
}
